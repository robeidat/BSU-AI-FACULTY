...
# Generate log income outcome with SEPARATE functions for each group
# Control group (no college): flatter relationship with parental income
intercept_control <- 3.5
slope_control <- 0.3    # Modest returns to family income

# Treated group (college): much steeper relationship  
intercept_treated <- 3.8
slope_treated <- 0.8    # College grads benefit much more from family advantages

# Generate outcomes using group-specific functions
log_income <- ifelse(has_degree == 0,
                     # No college: flatter income function
                     intercept_control + slope_control * log_parent_income + rnorm(n, 0, 0.15),
                     # College: steeper income function  
                     intercept_treated + slope_treated * log_parent_income + rnorm(n, 0, 0.15))
...
# Treatment effect = (intercept_treated + slope_treated * income) - (intercept_control + slope_control * income)
# = (3.8 + 0.8 * income) - (3.5 + 0.3 * income) = 0.3 + 0.5 * income

# Calculate average treatment effects
ate_full <- mean(0.3 + 0.5 * (data$log_parent_income - 10.8))
ate_overlap <- mean(0.3 + 0.5 * (data_overlap$log_parent_income - 10.8))

cat("\nInterpretation:\n")
cat("- SEPARATE OUTCOME FUNCTIONS by education group:\n")
cat("- No College:  log_income = 3.5 + 0.3 * log_parent_income + noise\n")
cat("- College:     log_income = 3.8 + 0.8 * log_parent_income + noise\n")
cat("- Treatment Effect = 0.3 + 0.5 * (log_parent_income - 10.8)\n\n")

# Show treatment effects at different income levels
cat("Treatment effects by family income:\n")
for(income in c(10.0, 10.8, 11.6)) {
  te <- 0.3 + 0.5 * (income - 10.8)
  cat(sprintf("- At log_parent_income = %.1f: %.1f%% return to college\n", income, te * 100))
}
cat("\n")
...
cat("- College graduates have STEEPER income-parental income relationship (0.8 vs 0.3)\n")
cat("- Full sample includes many high-income college grads (high outcomes)\n")  
cat("- But almost no high-income non-grads for comparison\n")
cat("- OLS conflates steeper slope with treatment effect\n")
cat("- Common support compares families with similar backgrounds\n")